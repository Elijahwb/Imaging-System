import { Action, getModule, Module, Mutation, VuexModule } from "vuex-module-decorators";
import store from '../'
import { Colors, EditorDimensions, FilterType, Photo, Shape, ShapeStyle, TextStyle } from '../../types'
import { getNumber, resizeElement, hexToRGBa } from '../helpers'

@Module({
    namespaced: true,
    name: 'Editing2d',
    dynamic: true,
    store
})
class Editing2dModule extends VuexModule {
    isCropping = false
    isAddingText = false
    isFreeDrawing = false
    isAddingShape = false
    image: Photo = {
        url: "data:image/png;base64,",
        type: 'Pano',
        name: 'default.png',
        created_at: '2021/11/10'
    }
    files: Array<Photo> = []
    imageEditor: any
    zoom = 1
    maxZoom = 2
    height = 0
    width = 0
    minZoom = 0.5
    loaded = false;
    activeObjectId = ""

    @Mutation
    setSavedImages(images: Array<Photo>) {
        this.files = images
    }

    @Action({ commit: 'setSavedImages', rawError: true })
    loadSavedImages() {
        const storedImages = localStorage.getItem("saved_images");
        return storedImages ? JSON.parse(storedImages) : [];
    }

    @Mutation
    loadImageFromURL() {
        this.imageEditor
            .invoke(
                "loadImageFromURL",
                this.image.url,
                "My sample image"
            )
            .then((result: EditorDimensions) => {
                this.resizeEditor(result);
            });
    }

    @Mutation
    resizeEditor(dimensions: EditorDimensions) {
        this.imageEditor
            .invoke("resize", {
                imageSize: {
                    oldWidth: dimensions.oldWidth,
                    oldHeight: dimensions.oldHeight,
                    newWidth: dimensions.newWidth,
                    newHeight: dimensions.newHeight,
                },
                uiSize: { width: dimensions.newWidth, height: dimensions.newHeight },
            })
            .then((result: Record<string, unknown>) => {
                console.log(result);
            });
    }

    @Mutation
    addText(data: {
        text: string,
        font?: { color: string, size: number, weight: "normal" | "bold" | undefined },
        position?: { x: number, y: number }
    }) {
        this.imageEditor.invoke("addText", data.text, {
            styles: {
                fill: data.font?.color ?? Colors.black,
                fontSize: data.font?.size ?? 20,
                fontWeight: data.font?.weight ?? undefined,
            },
            position: {
                x: data.position?.x ?? 10,
                y: data.position?.y ?? 10,
            },
        });
        this.isAddingText = true
        this.isCropping = false
    }

    @Mutation
    changeTextStyle(styles: TextStyle) {
        console.log({ styles });
        this.imageEditor.invoke("changeTextStyle", this.activeObjectId, styles
            // {
            //     fill: styles.fill,
            //     fontSize: styles.fontSize ?? 20,
            //     fontWeight: styles.fontWeight,
            //     fontStyle: styles.fontStyle ?? "normal",
            //     textDecoration: styles.textDecoration ?? undefined,
            //     textAlign: styles.textAlign ?? "center"
            // }
        )
    }

    @Mutation
    activateTextMode() {
        const mode = this.imageEditor.invoke("getDrawingMode");
        if (mode !== 'TEXT') {
            this.imageEditor.invoke("stopDrawingMode");
            this.imageEditor.invoke("startDrawingMode", "TEXT");
        }
        this.isAddingText = true
        this.isCropping = false
        this.isAddingShape = false
    }

    @Mutation
    activateShapeMode(shape: Shape) {
        const mode = this.imageEditor.invoke("getDrawingMode");
        if (mode !== 'SHAPE') {
            this.imageEditor.invoke("stopDrawingMode");
            this.imageEditor.invoke("startDrawingMode", "SHAPE",);
            this.imageEditor.invoke("setDrawingShape", shape.type, shape.options);
        }
        this.isAddingShape = true
        this.isAddingText = false
        this.isCropping = false
    }

    @Mutation
    rotateClockwise() {
        this.imageEditor.invoke("rotate", 10).then((status: number) => {
            console.log({ status });
        });
    }

    @Mutation
    rotateAntiClockwise() {
        this.imageEditor.invoke("rotate", -10).then((status: number) => {
            console.log({ status });
        });
    }

    @Mutation
    addShape(shape: Shape) {
        this.imageEditor
            .invoke("addShape", shape.type, shape.options
                //  {
                //     fill: { type: 'color', color: 'transparent' },
                //     stroke: "blue",
                //     strokeWidth: 3,
                //     width: 100,
                //     height: 200,
                //     left: 10,
                //     top: 10,
                //     isRegular: true,
                // }
            )
            .then((status: Record<string, unknown>) => {
                console.log({ status });
            });

    }

    @Mutation
    modeCropping() {
        const crop = this.imageEditor.invoke("startDrawingMode", "CROPPER");
        this.isCropping = crop
        this.isAddingText = false
        this.isFreeDrawing = false
        this.isAddingShape = false
    }

    @Mutation
    startDrawing(data: { width?: number, color?: string }) {
        this.imageEditor.invoke("stopDrawingMode");
        this.imageEditor.invoke("startDrawingMode", "FREE_DRAWING", {
            width: data.width ?? 5,
            color: data.color ? hexToRGBa(data.color, 0.5) : "rgba(255, 0, 0, 0.5)",
        });
        this.isAddingText = false
        this.isCropping = false
        if (!this.isFreeDrawing)
            this.isFreeDrawing = true
    }

    @Mutation
    showActions() {
        const actions = this.imageEditor.invoke("getActions");
        console.log({ actions });
        const mode = this.imageEditor.invoke("getDrawingMode");
        console.log({ mode });
        const cropRect = this.imageEditor.invoke("getImageName");
        console.log({ cropRect });
        const draw = this.imageEditor.invoke("setDrawingShape", "rect", {
            fill: {
                type: "filter",
                filter: [{ blur: 0.3 }, { pixelate: 20 }],
            },
            width: 100,
            height: 200,
        });
        console.log({ draw });
        const DrawingMode = this.imageEditor.invoke(
            "startDrawingMode",
            "LINE_DRAWING",
            {
                width: 10,
                color: "rgba(255,0,0,0.5)",
                arrowType: {
                    tail: "chevron", // triangle
                },
            }
        );
        console.log({ DrawingMode });
    }

    @Mutation
    async loadLocalFile(event: Event) {
        const files = (event.target as HTMLInputElement).files
        const image = files ? files[0] : null
        const dimensions = await this.imageEditor
            .invoke("loadImageFromFile", image, image?.name)
        this.zoom = 1;
        this.imageEditor.invoke("clearUndoStack");
        this.height = dimensions.newHeight
        this.width = dimensions.newWidth
        localStorage.removeItem('saved_images')
        this.files = []
        this.isAddingText = false
        this.isCropping = false
        this.isFreeDrawing = false
        this.isAddingShape = false
    }

    @Mutation
    crop() {
        try {
            const croppedArea = this.imageEditor.invoke("getCropzoneRect");
            if (croppedArea)
                this.imageEditor.invoke("crop", croppedArea).then(() => {
                    this.imageEditor.invoke("stopDrawingMode");
                    this.isCropping = false
                    this.zoomChange("in");
                });
        } catch (error) {
            console.log(error);

        }
        this.isAddingText = false
        this.isCropping = false
        this.isFreeDrawing = false
        this.isAddingShape = false
    }

    @Mutation
    saveFile() {
        const imageName = Date.now().toString();
        const dataURL = this.imageEditor.invoke("toDataURL");
        console.log(typeof dataURL);
        // var blob, type, w;
        // blob = base64ToBlob(dataURL);
        // type = blob.type.split("/")[1];
        // if (imageName.split(".").pop() !== type) {
        //   imageName += "." + type;
        // }
        this.files.push({
            url: dataURL,
            created_at: imageName,
            type: 'Pano',
            name: imageName + '.png'
        })
        localStorage.setItem("saved_images", JSON.stringify(this.files))
        // const url = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAYAAACNbyblAAAAHElEQVQI12P4//8/w38GIAXDIBKE0DHxgljNBAAO9TXL0Y4OHwAAAABJRU5ErkJggg=="

        // fetch(url)
        //     .then(res => res.blob())
        //     .then(console.log)
        // console.log({ TotalFiles: this.files });

        // const w = window.open();
        // if (w)
        //     w.document.body.innerHTML = '<img src="' + dataURL + '">';

        // // Library: FileSaver - saveAs
        // saveAs(blob, imageName); // eslint-disable-line
        this.isAddingText = false
        this.isCropping = false
        this.isFreeDrawing = false
        this.isAddingShape = false
    }

    @Mutation
    startLineDrawing() {
        this.imageEditor.invoke("startDrawingMode", "LINE_DRAWING", {
            width: 5,
            color: "rgba(255, 0, 0, 0.5)",
        });
        this.isAddingText = false
        this.isCropping = false
        this.isFreeDrawing = false
        this.isAddingShape = false
    }

    @Mutation
    zoomChange(type: string) {
        const updated = this.zoom;
        if (type === "in") {
            if (updated <= this.maxZoom) this.zoom = updated + 0.25;
            else
                return
        } else {
            if (updated >= this.minZoom) this.zoom = updated - 0.25;
            else
                return
        }
        if (!this.loaded) {
            const canvas = document.querySelector("canvas");
            if (canvas) {
                this.height = getNumber(canvas.style.maxHeight)
                this.width = getNumber(canvas.style.maxWidth)

                this.loaded = true;
            }
        }
        resizeElement(
            document.querySelector(".tui-image-editor-canvas-container"),
            this.zoom,
            ["height", "width"],
            [this.height, this.width]
        );
        resizeElement(
            document.querySelector("div.tui-image-editor-canvas-container"),
            this.zoom,
            ["maxHeight", "maxWidth"],
            [this.height, this.width]
        );
        document.querySelectorAll("canvas").forEach((element) => {
            resizeElement(
                element,
                this.zoom,
                ["maxHeight", "maxWidth"],
                [this.height, this.width]
            );
        });
    }

    @Mutation
    setImageToEdit(image: Photo) { this.image = image }

    @Mutation
    setupEditor(config: { editor: any, image: Photo }) {
        this.imageEditor = config.editor
        this.image = config.image
    }

    @Mutation
    changeShapeStype(styles: ShapeStyle) {
        this.imageEditor.invoke("changeTextStyle", this.activeObjectId, styles)
    }

    @Action({ commit: 'setupEditor', rawError: true })
    loadInitialEditor(config: { editor: any, image: Photo }) {
        console.log({ editor_id: config.editor._uid });
        // TO DO on refresh persist the image
        config.editor
            .invoke("loadImageFromURL", config.image.url, "My sample image")
            .then((result: Record<string, unknown>) => {
                // this.resizeEditor(result);
                console.log(result);
            });
        return config
    }

    @Mutation
    applyOrRemoveFilter(data: { applying: boolean, type: FilterType, options: Record<string, unknown> }) {
        if (data.applying) {
            this.imageEditor.invoke("applyFilter", data.type, data.options)
                .then((result: Record<string, unknown>) => {
                    console.log({ result });
                });
        } else {
            this.imageEditor.invoke("removeFilter", data.type)
                .then((result: Record<string, unknown>) => {
                    console.log({ RemovedFilter: result });
                });
        }

    }

    @Mutation
    setCurrentActiveObject(objId: string) {
        this.activeObjectId = objId
    }

    @Mutation
    undo() {
        this.imageEditor.invoke("undo", 1);
        // this.isAddingText = false
        // this.isCropping = false
        // this.isFreeDrawing = false
    }
    @Mutation
    redo() {
        this.imageEditor.invoke("redo", 1);
        // this.isAddingText = false
        // this.isCropping = false
        // this.isFreeDrawing = false
    }
    @Mutation
    async reset() {
        this.imageEditor.invoke("stopDrawingMode");
        await this.imageEditor.invoke("clearObjects");
        this.isAddingText = false
        this.isCropping = false
        this.isFreeDrawing = false
        this.isAddingShape = false
    }

    @Mutation
    cancleCropping() {
        this.imageEditor.invoke("stopDrawingMode");
        this.isCropping = false
    }

}

export default getModule(Editing2dModule)